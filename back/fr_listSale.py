...
class AgregarVentaDialog(wx.Dialog):
    def __init__(self, parent,gestion_ventas):
        ...
        # Cliente
        cliente_sizer = wx.BoxSizer(wx.HORIZONTAL)  # Sizer horizontal para el label, SearchCtrl y botón
        ...
        cliente_sizer.Add(self.txt_cliente, flag=wx.EXPAND | wx.LEFT | wx.RIGHT, border=10, proportion=1)

        self.btn_buscar_cliente = wx.Button(panel, label="Buscar Cliente")
        cliente_sizer.Add(self.btn_buscar_cliente, flag=wx.LEFT | wx.RIGHT | wx.ALIGN_CENTER_VERTICAL, border=5)
        self.btn_buscar_cliente.Bind(wx.EVT_BUTTON, self.filtrar_clientes)

        vbox.Add(cliente_sizer, flag=wx.EXPAND | wx.LEFT | wx.RIGHT | wx.TOP, border=10)
        ...
        # Ahora podemos decidir si mantenemos el Bind para Enter en el SearchCtrl
        # Si solo quieres usar el botón, puedes comentar o eliminar estas líneas:
        #self.txt_cliente.Bind(wx.EVT_TEXT_ENTER, self.filtrar_clientes)
        print("Evento wx.EVT_TEXT_ENTER vinculado a self.filtrar_clientes")
        self.txt_cliente.Bind(wx.EVT_KEY_DOWN, self.on_key_cliente) # También podrías eliminar esto

        # self.lista_clientes.Bind(wx.EVT_LISTBOX, self.seleccionar_cliente)

        self.cargar_clientes()
# Buscar Producto (usando wx.SearchCtrl)
        buscar_producto_sizer = wx.BoxSizer(wx.HORIZONTAL)
        vbox.Add(wx.StaticText(panel, label="Buscar Producto:"), flag=wx.LEFT | wx.TOP, border=10)

        self.txt_buscar_producto = wx.SearchCtrl(panel, style=wx.TE_PROCESS_ENTER)
        buscar_producto_sizer.Add(self.txt_buscar_producto, flag=wx.EXPAND | wx.LEFT | wx.RIGHT, border=10, proportion=1)

        self.btn_buscar_producto = wx.Button(panel, label="Buscar producto")
        buscar_producto_sizer.Add(self.btn_buscar_producto, flag=wx.LEFT | wx.RIGHT | wx.ALIGN_CENTER_VERTICAL, border=5)
        self.btn_buscar_producto.Bind(wx.EVT_BUTTON, self.filtrar_productos)

        vbox.Add(buscar_producto_sizer, flag=wx.EXPAND | wx.LEFT | wx.RIGHT | wx.TOP, border=10)

        self.txt_buscar_producto.Bind(wx.EVT_TEXT, self.filtrar_productos) # Filtrar mientras se escribe (opcional)
        self.txt_buscar_producto.Bind(wx.EVT_SEARCH, self.filtrar_productos) # Filtrar al presionar Enter o clic en botón de búsqueda
        # Lista de productos
    ...
    def ShowModal(self):
        self.cargar_productos()  # Recargar productos antes de mostrar
#        self.actualizar_lista_productos()
    ...
    def on_key_cliente(self, event):
        ...
        #keyname = wx.keyCodes.get(keycode, str(keycode))  # Obtener el nombre del diccionario o usar el 
        #print(dir(event))  # Lista todos los atributos y métodos
    ...
    def cargar_productos(self):
        ...
        if not isinstance(productos, dict):
            print("Error: 'productos' no es un diccionario. No se pueden cargar los productos.")
            return productos_dict  # O quizás lanzar una excepción, dependiendo de cómo quieras manejar el error
    ...
    def actualizar_lista_productos(self, productos, filtro=""):
        self.list_productos.Clear()
        ...
        if productos:
            for id_producto, producto in productos.items():
                try:
                    nombre = str(producto['nombre']).lower()
                    filtro_lower = filtro.lower()
                    if filtro_lower in nombre:
    ...
    def filtrar_productos_noanda(self, event):
        ...
        productos = gestion_producto.obtener_productos()
        #self.actualizar_lista_productos(productos, filtro)
    ...
    def filtrar_productos(self, event):
        ...
        if productos_filtrados:
            for id_producto, datos in productos_filtrados.items():
                item_text = f"Código: {id_producto} - {datos['nombre']} - Stock: {datos['stock']} - Precio: ${datos['precio']}"
                self.list_productos.Append(item_text)            

        return productos_filtrados # Opcional: Devolver los productos filtrados


         #agregar un solo producto a la lista 
